...
| `read-skill` | `skills/reading/read-skill` | 读者 | 读一个第三方 skill 或仓库，产出带结论的用法导读 |
...
每个 skill 的入口文件都是 `SKILL.md`，其中 `name` 必须与所在目录名一致。八套 skill 都带有 `disable-model-invocation: true`，只能主动触发。`scholar`、`investigate` 和阅读 skills 都会按名字引用 `technical-diagrams` 与 `scholarly-standards`。
