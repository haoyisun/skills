...
skill 自己的协议放在 `references/` 目录里：目标模型、术语与知识补丁、输出 schema、审校清单。模板放在 `assets/`，两个 Node 脚本分别负责搭建会话骨架和检查完成的会话。信源评估、写作、图片、能力与提问规则来自共享标准层。

## 一套调研 skill，三种交付档位

`investigate` 的起点是决策问题而不是学习目标，产物写到 `.scholar/research/`。同一个入口分流到三个档位：

- `brief` 简报：结论与关键证据，单文件。
- `plan` 方案：完整的设计方案与实施方案，单文件，是默认档位。
- `dossier` 深研：包含调研发现、候选方案、设计、实施与附录的多文件产物。

它的协议放在 `references/` 里：调研任务卡、调研方法、证据与实验、输出 schema、决策点与置信度、审校清单，另外按名字引用共享标准层与 `technical-diagrams`。
...
把产物与用户项目源码分开，避免污染仓库。`.scholar/` 下按类型再分 `projects/`、`deep/`、`quick/`、`study/`、`research/`，方便查找、清理和避免命名冲突。`.reader/` 是改名前的根目录，已有产物留在原位。
