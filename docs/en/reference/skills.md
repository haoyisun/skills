...
Each skill's entry point is `SKILL.md`. Its `name` field must match the directory name. All seven skills have `disable-model-invocation: true`, so they are explicit-only. `scholar`, `investigate`, and the reading skills reference `technical-diagrams` and `scholarly-standards` by name.
