...
`read-project`, `read-standard`, and `read-fast` are the Reader family. They depend on `technical-diagrams` for their diagrams. Install it together with them, or install the whole repository to get them all at once.
