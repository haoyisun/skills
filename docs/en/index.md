English · [简体中文](../zh/index.md)

# Scholar

**Scholar** (学者) is a collection of agent skills for reading, learning, and understanding technical material with an AI tool.

The Reader (读者) family reads material that already exists:

- `/read-project`: understand and get started with a software project
- `/read-standard`: learn a technical article, blog post, book, or PDF deeply
- `/read-fast`: get a quick overview in 2–10 minutes

The Scholar family produces learning material from a goal:

- `/scholar`: learn a concept or knowledge system, from a ten-minute overview to a multi-week handbook

## Documentation

- [Tutorials](tutorials/index.md): complete your first setup in order
- [How-to guides](how-to/index.md): solve a specific task
- [Reference](reference/index.md): look up skills, formats, and output conventions
- [Explanation](explanation/index.md): understand the project design
